...
impl AnalysisTable {
    ...
    pub fn get_expected_tokens(&self, non_terminal: &NonTerminal) -> Vec<&Token> {
        if let Some(map) = self.table.get(non_terminal) {
            map.keys()
                .map(|d| self.tokens_discriminants.get(d).unwrap())
                .collect()
        } else {
            Vec::new()
        }
    ...
    fn try_set(
        &mut self,
        left_non_terminal: &ParsedLexem,
        token: &Token,
        production: &Vec<ParsedLexem>,
    ) {
        let non_terminal = match &left_non_terminal.lexem {
            Lexem::Terminal(t) => panic!(
                "Trying to fill analysis table with the terminal {} on the left of a rule",
                t
            ),
        ...
        self.tokens_discriminants
            .insert(discriminant(token), token.clone());
    }
}
...
impl From<&Grammar> for AnalysisTable {
    fn from(grammar: &Grammar) -> Self {
        ...
        let analysis_table: HashMap<NonTerminal, HashMap<Discriminant<Token>, Vec<Lexem>>> =
            HashMap::new();

        let mut table = AnalysisTable {
            table: analysis_table,
            tokens_discriminants: HashMap::new(),
            used_non_terminals: grammar
                .non_terminal_lexems
                .iter()
