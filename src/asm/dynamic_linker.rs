...
pub fn get_dynamic_linker(target_machine: &TargetMachine) -> String {
    ...
    let (lib_dir, loader) = match true {
        ...
        _ if is_arch("arm") || is_arch("thumb") => {
            ...
            (
                "lib".to_string(),
                if hf {
                    "ld-linux-armhf.so.3".to_string()
                } else {
                    "ld-linux.so.3".to_string()
                },
            )
        }
        _ if is_arch("mips64el") => {
            let nan2008 = triple_str.contains("nan2008");
            let lib_suffix = if triple_str.contains("n32") {
                "32"
            } else {
                "64"
            };
            ...
            if triple_str.contains("uclibc") {
                (
                    ...
                    if nan2008 {
                        "ld-uClibc-mipsn8.so.0".to_string()
                    } else {
                        "ld-uClibc.so.0".to_string()
                    },
                )
            } else {
                (
                    format!("lib{}", lib_suffix),
                    if nan2008 {
                        "ld-linux-mipsn8.so.1".to_string()
                    } else {
                        "ld.so.1".to_string()
                    },
                )
        ...
        _ if is_arch("ppc") => ("lib".to_string(), "ld.so.1".to_string()),
        ...
        _ if is_arch("sparc") => ("lib".to_string(), "ld-linux.so.2".to_string()),
        _ if is_arch("s390x") || is_arch("systemz") => ("lib".to_string(), "ld64.so.1".to_string()),
        _ if is_arch("x86_64") => {
    ...
    if triple_str.contains("exherbo") && (!triple_str.contains("-") || triple_str.contains("-pc-"))
    {
