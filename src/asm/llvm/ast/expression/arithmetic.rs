use crate::{asm::{codegen::CodeGen, llvm::{assert_type, smolvar::SmolVar, LLVMCodegenError}}, typing::Type};

pub fn compute_mult<'ctx>(x: SmolVar<'ctx>, y: SmolVar<'ctx>, cg: &mut CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    assert_type(Type::Int, &x, cg, None)?;
    assert_type(Type::Int, &y, cg, None)?;
    return compute_mult_unchecked(x, y, cg);
}

pub fn compute_mult_unchecked<'ctx>(x: SmolVar<'ctx>, y: SmolVar<'ctx>, cg: &mut CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
...
pub fn compute_div_unchecked<'ctx>(x: SmolVar<'ctx>, y: SmolVar<'ctx>, cg: &mut CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    let x_val = cg.get_variable_value(x)?.into_int_value();
    let y_val = cg.get_variable_value(y)?.into_int_value();
...
pub fn compute_mod_unchecked<'ctx>(x: SmolVar<'ctx>, y: SmolVar<'ctx>, cg: &mut CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    let x_val = cg.get_variable_value(x)?.into_int_value();
    let y_val = cg.get_variable_value(y)?.into_int_value();
...
pub fn compute_sub_unchecked<'ctx>(x: SmolVar<'ctx>, y: SmolVar<'ctx>, cg: &mut CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    let x_val = cg.get_variable_value(x)?.into_int_value();
    let y_val = cg.get_variable_value(y)?.into_int_value();
...
pub fn compute_add_unchecked<'ctx>(x: SmolVar<'ctx>, y: SmolVar<'ctx>, cg: &mut CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    let x_val = cg.get_variable_value(x)?.into_int_value();
    let y_val = cg.get_variable_value(y)?.into_int_value();
