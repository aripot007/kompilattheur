...
pub fn llvm_compute_binop<'ctx>(e1: &Expression, op: &BinOp, e2: &Expression, cg: &mut CodeGen<'ctx>, root: &Expression) ->  Result<SmolVar<'ctx>, LLVMCodegenError> {
    match op {
        ...
        | BinOp::DIV
        | BinOp::MOD
        | BinOp::SUB => return llvm_compute_arithmetic(e1, op, e2, cg, root),

        // TODO : Remove when generic add is implemented
        BinOp::ADD if e1.get_type() == &Type::Int && e2.get_type() == &Type::Int => return llvm_compute_arithmetic(e1, op, e2, cg, root),

        BinOp::ADD => {
            cg.errors.push(Diagnostic::unimplemented_llvm(root));
            Err(LLVMCodegenError::Unimplemented(format!("ADD operation not implemented yet")))
        }

        BinOp::ACCESS => {
            cg.errors.push(Diagnostic::unimplemented_llvm(root));
            Err(LLVMCodegenError::Unimplemented(String::from("ACCESS operation not implemented yet")))
        },
    }
}

fn llvm_compute_arithmetic<'ctx>(e1: &Expression, op: &BinOp, e2: &Expression, cg: &mut CodeGen<'ctx>, root: &Expression) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
