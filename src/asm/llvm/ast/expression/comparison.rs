...
pub fn compare_int_values<'ctx>(value1: SmolVar<'ctx>, value2: SmolVar<'ctx>, operation: BinOp, cg: &CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    ...
    let res = match operation {
        ...
        _ => return Err(LLVMCodegenError::InvalidOperation(format!("Invalid between two ints: {:?}", operation))),
    };
...
pub fn compare_none_values<'ctx>(value1: SmolVar<'ctx>, value2: SmolVar<'ctx>, operation: BinOp, cg: &CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    let res = match operation {
        BinOp::EQ => cg.create_variable(
            Type::Bool,
            cg.context.bool_type().const_int(1, false)
        ),
        BinOp::NEQ => cg.create_variable(
            Type::Bool,
            cg.context.bool_type().const_int(0, false)
        ),
        _ => return Err(LLVMCodegenError::InvalidOperation(format!("Invalid between two None: {:?}", operation))),
    };
    return res;
}
...
pub fn compare_boolean_values<'ctx>(value1: SmolVar<'ctx>, value2: SmolVar<'ctx>, operation: BinOp, cg: &CodeGen<'ctx>) -> Result<SmolVar<'ctx>, LLVMCodegenError> {
    ...
    let val2 = cg.get_variable_value(value2)?.into_int_value();
    let res = match operation {
        BinOp::EQ => cg.builder.build_int_compare(inkwell::IntPredicate::EQ, val1, val2, "bool_eq")?,
        BinOp::NEQ => cg.builder.build_int_compare(inkwell::IntPredicate::NE, val1, val2, "bool_neq")?,
        BinOp::LESS => cg.builder.build_int_compare(inkwell::IntPredicate::ULT, val1, val2, "bool_lt")?,
        BinOp::GREATER => cg.builder.build_int_compare(inkwell::IntPredicate::UGT, val1, val2, "bool_gt")?,
        BinOp::LESSEQ => cg.builder.build_int_compare(inkwell::IntPredicate::ULE, val1, val2, "bool_lte")?,
        BinOp::GREATEREQ => cg.builder.build_int_compare(inkwell::IntPredicate::UGE, val1, val2, "bool_gte")?,
        _ => return Err(LLVMCodegenError::InvalidOperation(format!("Invalide between booleans : {:?}", operation))),
    };
    return cg.create_variable(Type::Bool, res);
}
