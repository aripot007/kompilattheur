...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub fn create_list(&self, capacity: IntValue<'ctx>) -> Result<SmolList<'ctx>, LLVMCodegenError> {
        let array_ptr = self.builder.build_array_alloca(self.smolpp_types.dynamic_type, capacity, "list_array")?;
        return self.create_list_struct(capacity, array_ptr);
    }

    /// Create a list in the heap and return the pointer to it
    pub fn create_list_in_heap(&self, capacity: IntValue<'ctx>) -> Result<PointerValue<'ctx>, LLVMCodegenError> {

        let array_ptr = self.builder.build_array_malloc(self.smolpp_types.dynamic_type, capacity, "list_heap_array")?;
        let list_struct = self.create_list_struct(capacity, array_ptr)?;

        // Store the structure in the heap
        let list_ptr = self.builder.build_malloc(self.smolpp_types.list_type, "list")?;
        self.builder.build_store(list_ptr, list_struct)?;
        return Ok(list_ptr);
    }

    /// Create a list with the given capacity in the stack
    fn create_list_struct(&self, capacity: IntValue<'ctx>, array_ptr: PointerValue<'ctx>) -> Result<SmolList<'ctx>, LLVMCodegenError> {

        let undef = self.smolpp_types.list_type.get_undef();
        let len = self.context.i64_type().const_zero();
        let with_len = self.builder.build_insert_value(undef, len, 0, "with_len")?.into_struct_value();
        let with_capa = self.builder.build_insert_value(with_len, capacity, 1, "with_capa")?.into_struct_value();
        let full_struct = self.builder.build_insert_value(with_capa, array_ptr, 2, "full_struct")?.into_struct_value();
    ...
    pub fn create_list_variable(&self, capacity: IntValue<'ctx>, heap: bool) -> Result<(SmolVar<'ctx>, PointerValue<'ctx>), LLVMCodegenError> {
        ...
        let list_ptr = match heap {
            ...
            false => {
                ...
                let list_ptr = self.builder.build_alloca(self.smolpp_types.list_type, "list")?;
                self.builder.build_store(list_ptr, list_struct)?;
                list_ptr
            }
        };
    ...
    pub fn get_list_length(&self, list: SmolList<'ctx>) -> Result<IntValue<'ctx>, LLVMCodegenError> {
        return Ok(self.builder.build_extract_value(list, 0, "list_len")?.into_int_value());
    }

    fn set_list_length(&self, list: SmolList<'ctx>, len: IntValue<'ctx>) -> Result<SmolList<'ctx>, LLVMCodegenError> {
        return Ok(self.builder.build_insert_value(list, len, 0, "set_list_len")?.into_struct_value());
    }

    fn get_list_capacity(&self, list: SmolList<'ctx>) -> Result<IntValue<'ctx>, LLVMCodegenError> {
        return Ok(self.builder.build_extract_value(list, 1, "list_cap")?.into_int_value());
    }

    fn set_list_capacity(&self, list: SmolList<'ctx>, capacity: IntValue<'ctx>) -> Result<SmolList<'ctx>, LLVMCodegenError> {
