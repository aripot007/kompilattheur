...
pub fn smolpp_panic<'ctx>(cg: &CodeGen<'ctx>, error: RuntimeErrorMsg, fmt_args: &[BasicMetadataValueEnum<'ctx>]) {
    ...
    cg.builder.build_call(trap_func, &[], "call_trap").unwrap();
}
...
pub fn smolpp_panic_with_unreachable<'ctx>(cg: &CodeGen<'ctx>, error: RuntimeErrorMsg, fmt_args: &[BasicMetadataValueEnum<'ctx>]) {
