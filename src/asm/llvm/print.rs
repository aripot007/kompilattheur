...
use super::panic::{smolpp_panic, smolpp_panic_with_unreachable};
...
pub fn print_int_value<'ctx>(variable: &StructValue<'ctx>, cg: &CodeGen<'ctx>) {
    ...
    let value = cg.builder.build_extract_value(*variable, 1, "value_field").unwrap().into_int_value();
...
pub fn print_string_value<'ctx>(variable: &StructValue<'ctx>, cg: &CodeGen<'ctx>) {
    let ptr_type = cg.context.ptr_type(AddressSpace::default());

    let value_generic = cg.builder.build_extract_value(*variable, 1, "value_field").unwrap();

    let value = match value_generic.is_int_value() {
        true => cg.builder.build_int_to_ptr(value_generic.into_int_value(), ptr_type, "str_ptr").unwrap(),
...
pub fn print_list_value<'ctx>(value: &StructValue<'ctx>, cg: &CodeGen<'ctx>) {
    smolpp_panic(cg, RuntimeErrorMsg::PanicNotImplemented, &[]);
}

/// Generate LLVM to print any value.
pub fn print_any_value<'ctx>(value: &StructValue<'ctx>, cg: &CodeGen<'ctx>) {
    cg.builder.build_call(get_internal_func!(cg, InternalFuctions::GenericPrint), &[value.as_basic_value_enum().into()], "generic_print_call").unwrap();
}

/// Initialize the internal "generic_print" function that can print any value
pub fn init_internal_generic_print_function<'ctx>(cg: &mut CodeGen<'ctx>) {
    ...
    let type_field = cg.builder.build_extract_value(param, 0, "type_field").unwrap().into_int_value();
