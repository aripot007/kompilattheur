...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub fn get_variable_type(
        ...
        let val = self
            .builder
            .build_extract_value(variable, 0, "type_field")?
            .into_int_value();
        return Ok(val);
    }
    ...
    pub fn get_variable_value(
        &self,
        variable: SmolVar<'ctx>,
    ) -> Result<BasicValueEnum<'ctx>, LLVMCodegenError> {
        let val = self
            .builder
            .build_extract_value(variable, 1, "value_field")?;
        return Ok(val);
    }
        ...
        BV: BasicValue<'ctx>,
    {
        let res = self
            .builder
            .build_insert_value(variable, value, 1, "set_value")?;
        return Ok(res.into_struct_value());
    }

    pub fn create_variable<BV>(&self, t: Type, value: BV) -> Result<SmolVar<'ctx>, LLVMCodegenError>
    where
        BV: BasicValue<'ctx>,
    {
        let undef = self.smolpp_types.dynamic_type.get_undef();
        let var_type_discr_val = self
            .context
            .i8_type()
            .const_int(t.get_bitmask() as u64, false);
        let with_type = self
            .builder
            .build_insert_value(undef, var_type_discr_val, 0, "with_type")?
            .into_struct_value();
        let full_struct = self
            .builder
            .build_insert_value(with_type, value.as_basic_value_enum(), 1, "with_value")?
            .into_struct_value();
