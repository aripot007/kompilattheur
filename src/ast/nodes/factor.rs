use crate::{analysis_table::get_analysis_table, common::types::{file_element::file_element_from, FileElement, Node, NumToken, Token, Tree}, parser::Lexem};
    ...
    fn from(root: Tree<FileElement<Lexem>>) -> Self {
        ...
        match non_terminal {
            NonTerminal::Const => {
                ...
                match val.element {
                    Lexem::Terminal(Token::Integer(NumToken {value})) => return Factor::Integer(file_element_from!(val, value)),
                    Lexem::Terminal(Token::String(string)) => return Factor::String(file_element_from!(val, string)),
                    Lexem::Terminal(Token::True) => return Factor::True(file_element_from!(val, Token::True)),
                    Lexem::Terminal(Token::False) => return Factor::False(file_element_from!(val, Token::False)),
                    Lexem::Terminal(Token::None) => return Factor:: None(file_element_from!(val, Token::None)),
                    Lexem::Terminal(token) => panic!("Malformed CST: Expected NumToken while parsing const, found {}", token),
                    Lexem::NonTerminal(id) => panic!("Malformed CST: Expected NumToken while parsing const, found {}", table.get_non_terminal_name(id)),
                }
            },
