...
pub(super) struct SmolFilledList {}

impl SmollibFunction for SmolFilledList {
    fn name(&self) -> &str {
        "filled"
    }
    ...
    fn llvm_type<'ctx>(&self, cg: &CodeGen<'ctx>) -> FunctionType<'ctx> {
        ...
        let func_type = var_type.fn_type(&vec![var_type.into(), var_type.into()], false);
        return func_type;
    }
