...
use crate::{
    ast::nodes::{BinOp, Expression, UnOp},
    ...
    fn parse_type(&self, context: &mut TypingContext) -> Result<Type, ()> {
        match self {
            Expression::BINOP(e1, bin_op, e2) => {
                try_parse_binop(self, e1.as_ref(), *bin_op, e2.as_ref(), context)
            }
            ...
            Expression::NotImplemented => {
                context.errors.push(Diagnostic::from_localizable_ref(
                    ...
                    format!("Unimplemented expression encountered during type checking"),
                ));
                Err(())
            }
        }
        ...
        BinOp::EQ | BinOp::NEQ => {
                ...
                && !(t1 == Type::Bool && t2 == Type::Int)
            {
                match (&t1, &t2) {
                    ...
                    _ => context
                        .warnings
                        .push(Diagnostic::dubious_comparison(root, &t1, &t2, false)),
                }
            }
            return Ok(Type::Bool);
        }
