use crate::{ast::nodes::{Factor, FactorKind}, common::{diagnostic::Diagnostic, localizable::{localization_info, Localizable, LocalizationInfo}}, typing::{Type, Typeable, TypingContext}};
        ...
        let res = match &mut self.kind {
            ...
            FactorKind::Call { identifier, args, localization: _ } => {
                ...
                let mut err = match func_type_res {
                    Some(_) => false,
                    None => {
                        context.errors.push(Diagnostic::unknown_symbol(&localization, &identifier.name));
                        true
                    }
                };
